...
mod tests {
    ...
    fn get_tickers_test() {
        ...
        let mock_get_market_data = server
            .mock("GET", "/api/v5/market/tickers")
    ...
    fn get_index_tickers_test() {
        let mut server = Server::new();
        let mock_get_market_data = server
            .mock("GET", "/api/v5/market/index-tickers")
            .with_header("content-type", "application/json")
    ...
    fn get_order_book_lite_test() {
        let mut server = Server::new();
        let mock_get_market_data = server
            .mock("GET", "/api/v5/market/books-lite")
            .with_header("content-type", "application/json")
            .match_query(Matcher::Regex("instId=BTC-USDT".into()))
            .with_body_from_file("tests/mocks/market_data/get_order_book.json")
            .create();
    ...
    fn get_index_candles_test() {
        let mut server = Server::new();
        let mock_get_market_data = server
            .mock("GET", "/api/v5/market/index-candles")
            .with_header("content-type", "application/json")
            .match_query(Matcher::Regex("instId=BTC-USDT".into()))
